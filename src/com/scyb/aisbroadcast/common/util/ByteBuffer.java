...
public class ByteBuffer {
	...
	private void expandCapacity(int minimumCapacity) {
		...
		if(minimumCapacity > newCapacity) newCapacity = minimumCapacity;
	...
	public byte[] getValueN(int start, int length) {
		if((start + length) > count) return null;
	...
	public int resetValue(byte val) {
		for(int i = 0; i < count; i++) value[i] = val;
		return count;
	}

	public int resetValue(int start, int size, byte val) {
		if(start > (count - 1)) return 0;
		if((start + size) > count) return 0;
		for(int i = 0; i < size; i++) value[i + start] = val;
		return size;
	}

	public int replace(int loc, byte val) {
		if(loc > (count - 1)) return 0;
		value[loc] = val;
		return 1;
	}

	public int replace(int loc, byte[] val, int len) {
		if(loc > (count - 1)) return -1;
		if((loc + len) > count) return -1;
	...
	public ByteBuffer insert(int loc, byte value){
		return insert(loc,new byte[]{value});
	}
	...
	public byte getCheckSum() {
		byte v = 0x00;
		for(int i = 0; i < count; i++) v ^= value[i];
		return v;
	}
